...
class AttachedAchievement extends FlxSprite {
	...
	override function update(elapsed:Float) {
		if (sprTracker != null)
			setPosition(sprTracker.x - 160, sprTracker.y + 10);
...
class AchievementObject extends FlxSpriteGroup {
	...
	public function new(name:String, ?camera:FlxCamera = null)
	{
		...
		var achievementBG:FlxSprite = new FlxSprite(0, 0).makeGraphic(420, 150, FlxColor.BLACK);
		achievementBG.scrollFactor.set();

		var achievementIcon:FlxSprite = new FlxSprite(achievementBG.x + 5, achievementBG.y + 5).loadGraphic(Paths.image('achievements/' + name));
		...
		introTween = FlxTween.tween(this, {y: -50}, 0.3, {onComplete: function (twn:FlxTween){
			introTween = FlxTween.tween(this, {y: 0}, 0.5);
		}});

		introTween = FlxTween.tween(this, {y: -100}, 0.3, {startDelay: 3, 
			onComplete: function (twn:FlxTween)
				{introTween = FlxTween.tween(this, {y: -150}, 0.5, 
				{onComplete: function (twn:FlxTween){		
					introTween = null;
					remove(this);
					alpha = 0; //idkkk it goes fucky whenever its removed 
