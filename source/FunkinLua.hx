...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "windowTweenPos", function(x:Dynamic, y:Dynamic, duration:Float, ease:String) {
				FlxTween.tween(Application.current.window, {x: x, y: y}, duration, {ease: getFlxEaseByString(ease)});
		});
		...
		Lua_helper.add_callback(lua, "noteTweenScale", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			...
			if(testicle != null) {
				PlayState.instance.modchartTweens.set(tag, FlxTween.tween(testicle.scale, {x: value, y: value}, duration, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.callOnLuas('onTweenCompleted', [tag]);
						PlayState.instance.modchartTweens.remove(tag);
					}
		...
		Lua_helper.add_callback(lua, "windowTweenY", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			cancelTween(tag);
			if(note < 0) note = 0;
			var testicle:StrumNote = PlayState.instance.strumLineNotes.members[note % PlayState.instance.strumLineNotes.length];
